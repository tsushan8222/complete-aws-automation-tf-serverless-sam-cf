...
phases:
  ...
  pre_build:
    commands:
      ...
      - DEFAULT_CLEAN_BRANCH_NAME=$(echo $DEFAULT_BRANCH | sed -E "s/[^(release|hotfix)]+.$//g" | sed "s/hotfix/release/g")
      - DEFAULT_CLEAN_BRANCH_NAME=$(echo $${DEFAULT_CLEAN_BRANCH_NAME:0:20} | awk '{print tolower($0)}' | sed "s/\./-/g" | sed "s/$//g")
      ...
      - if [[ -z $BRANCH_NAME ]] ; then export BRANCH_NAME=$BRANCH_NAME_LOCAL ; else export BRANCH_NAME=$BRANCH_NAME ; fi
      ...
      - export CFN_STACK_NAME_LOCAL=$(echo "$ENV-$REPO_NAME-$BRANCH_NAME" | awk '{print tolower($0)}' )
