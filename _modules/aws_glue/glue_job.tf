...
resource "aws_glue_job" "glue_job" {
  ...
  connections            = length(var.glue_job_connections) > 0 ? var.glue_job_connections : (length(var.aws_glue_connections) > 0 ? concat(var.glue_job_additional_connections, [element(concat(aws_glue_connection.glue_connection.*.id, [""]), 0)]) : [])
