...
def get_code_build_dir():
    try:
        ...
        current_dir_location = os.path.join(current_dir_location,os.getenv('SERVICE_BASE_PATH',''))
...
def execute_script(script_path):
    ...
    try:
        ...
        sys.exit(1)
    try:
        run_script = subprocess.run(script_path, shell=True, check=True)
    except Exception as Error:
...
def run_serverless_deploy(serverless_yaml_dir, env, region):
    ...
    try:
        run_serverless_deploy = subprocess.run(
            ["serverless", "deploy", "-s", env, "-r", region, "-v"], cwd=serverless_yaml_dir, check=True)
    except Exception as Error:
