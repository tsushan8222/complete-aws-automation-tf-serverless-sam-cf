...
locals {
  ...
  queue_name              = "${local.global_environment_vars.locals.environment}-factfind-queue-dlq.fifo"
}

inputs = {
  env                 = local.global_environment_vars.locals.environment
  alarm_name          = "${local.global_environment_vars.locals.environment}-factfind-monitoring-sqs-dlq-alert"
