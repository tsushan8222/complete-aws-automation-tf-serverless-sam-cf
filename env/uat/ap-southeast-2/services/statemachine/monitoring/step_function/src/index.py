...
def lambda_handler(event, context):
    try:
        ...
        notify(to=os.environ['INTERNAL_GROUP_EMAIL'],data= step_function_exc,emailType = 'INTERNAL_ENQUIRY_ERROR')
        notify(to=os.environ['EXTERNAL_GROUP_EMAIL'],data= step_function_exc,emailType = 'ASSESTIVE_ENQUIRY_ERROR')
        return {
            'statusCode': 200,
...
def notify(to,data,emailType):
    try:
        response = requests.post(os.environ['NOTIFICATION_API_URL'], json={"receipentEmailAddress": to, "emailType": emailType, "data": data} ,headers = {"Content-Type": "application/json"})
        return response
    except Exception as error:
